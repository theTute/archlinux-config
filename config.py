...
screens = [
    ...
    Screen(
        top=bar.Bar(
            [
                widget.TextBox(" Groups ", background=ColorNormal),
                widget.Sep(foreground=ColorContrast),
                widget.GroupBox(
                    disable_drag=True,
                    highlight_method='line',
                    highlight_color=ColorNormal,
                    this_current_screen_border=ColorContrast,
                    ),
                widget.Sep(foreground=ColorContrast),
                ...
                widget.Spacer(),
                widget.Sep(foreground=ColorContrast),
                widget.Systray(background=ColorNormal),
                widget.Spacer(5, background=ColorNormal),
                widget.Sep(foreground=ColorContrast),
                widget.Clock(
                    format="%y/%m/%d %a %H:%M ",
...
def autostart():
    subprocess.run(['xwallpaper', '--maximize', '~/Pictures/Wallpaper'])
    processes = [
            ['picom'], 
            ['discord']
        ]
